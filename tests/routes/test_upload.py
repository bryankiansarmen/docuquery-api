...
def clean_document_store():
    ...
    store_mod.DOCUMENT_STORE.clear()
...
def test_upload_pdf_already_active_in_memory(client):
    ...
    store_mod.DOCUMENT_STORE.setdefault("default", {}).update({
...
def test_upload_pdf_already_in_redis(client, mocker):
    ...
    # Verify the tenant-scoped memory store was updated from cache
    assert store_mod.DOCUMENT_STORE["default"]["document_id"] == document_id
